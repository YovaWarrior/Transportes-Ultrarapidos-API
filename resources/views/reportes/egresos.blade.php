...
@section('title', 'Reporte de Egresos por Predio')
...
<div class="py-6">
  <div class="px-4 mx-auto max-w-7xl sm:px-6 md:px-8">
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold text-gray-900">Egresos por Predio</h1>
    ...
    <div class="mb-6 bg-white rounded-lg shadow p-4">
      <form method="GET" class="grid grid-cols-1 gap-4 sm:grid-cols-4">
        ...
        <div class="flex items-end">
          <button class="w-full px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700">Filtrar</button>
        </div>
      </form>
    </div>
      ...
      <div class="overflow-x-auto">
        ...
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Kilometraje</th>
          </tr>
        </thead>
        <tbody class="bg-white divide-y divide-gray-200">
          @forelse($egresos as $eg)
            <tr class="hover:bg-gray-50">
              <td class="px-6 py-4 text-sm text-gray-900">{{ $eg->fecha_egreso?->format('d/m/Y H:i') }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">#{{ $eg->ordenTrabajo->numero_orden ?? 'N/A' }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">{{ $eg->ordenTrabajo->camion->placa ?? 'N/A' }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">{{ $eg->ordenTrabajo->predio->nombre ?? '—' }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">{{ $eg->destino }}</td>
              <td class="px-6 py-4 text-sm text-gray-700">{{ $eg->tipo_carga }}</td>
              <td class="px-6 py-4 text-sm text-gray-900 text-right">{{ $eg->kilometraje ?? '—' }}</td>
            </tr>
          @empty
            <tr>
